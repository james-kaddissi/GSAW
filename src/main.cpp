...
namespace ui = gs::ui;
using gs::ui::UIScaleMode;
using gs::ui::UITextElement;
using gs::ui::UIWaveformElement;
using gs::ui::UIWaveformSource;
namespace StyleStates = gs::ui::StyleStates;
using gs::ui::DragPayload;
using gs::ui::StyleOverrides;

using namespace gs::audio;

class GSAWAppLayer final : public gs::core::GLayer
{
  public:
    explicit GSAWAppLayer(gs::ui::UILayer &uiLayer, AudioLayer &audioLayer) : m_uiLayer(uiLayer), m_audio(audioLayer)
    {
    }
    ...
    void onBypassClicked()
    {
        ...
        if (active)
            statusText->removeState(StyleStates::Muted);
        else
            statusText->addState(StyleStates::Muted);
    }

    void onOpenAllEditors()
    {
        if (m_audio.getVST3Host())
            m_audio.getVST3Host()->OpenAllPluginEditors(m_owner);
    }
    ...
    void onAttach(gs::core::AppContext &ctx) override
    {
        ...
        m_uiLayer.getLayoutRoot().setOnDrop([this](DragPayload p) { m_browserPanel->handleDrop(std::move(p)); });
        fpsText = ui::Text("FPS: --").size(14).color({0.7f, 0.7f, 0.7f, 1});
        statusText = ui::Text("Active")
                         .size(14)
                         .color({0.5f, 0.9f, 0.5f, 1.0f})
                         .style(StyleStates::Muted, gs::ui::StyleOverrides::TextColor({0.9f, 0.4f, 0.4f, 1.0f}));
        ...
        channelRackWidget->heightMode = gs::ui::UISizeMode::Flex;

        auto mixerWidget = m_mixerView->widget();
        mixerWidget->flexGrow = 1.0f;
        mixerWidget->heightMode = gs::ui::UISizeMode::Flex;
        ...
        auto rightColumn =
            ui::VStack()
                ...
                .build();
        rightColumn->crossAxisAlignment = gs::ui::UICrossAxisAlignment::Stretch;
        rightColumn->flexGrow = 1.0f;
        rightColumn->heightMode = gs::ui::UISizeMode::Fill;

        auto browserWidget = m_browserPanel->widget();
        browserWidget->flexGrow = 1.0f;
        browserWidget->heightMode = gs::ui::UISizeMode::Flex;
        auto leftColumn = ui::VStack().spacing(0).pad(0).width(200).add(browserWidget).add(inputWaveform).build();
        leftColumn->crossAxisAlignment = gs::ui::UICrossAxisAlignment::Stretch;
        leftColumn->heightMode = gs::ui::UISizeMode::Fill;

        auto mainContent = ui::HStack().spacing(4).pad(0).add(leftColumn).add(rightColumn).build();
        ...
        auto bottomBar = ui::HStack().height(10).pad(0).build();

        bottomBar->crossAxisAlignment = gs::ui::UICrossAxisAlignment::Stretch;

        auto root = ui::VStack().spacing(4).pad(0).add(titlebar).add(mainContent).add(bottomBar).build();
    ...
    void onEvent(gs::core::AppContext &ctx, const gs::core::AppEvent &e) override
    {
        if (e.type == gs::core::AppEvent::Type::Resize)
            ctx.requestRender();
    }
  ...
  private:
    void setupChannelRack()
    {
        ...
        TrackId synthTrackId =
            engine->createTrack("Synth", 2, {gs::audio::InputSource::Kind::None, 0, 0}, kMasterBusId);
...
int main()
{
    try
    {
        gs::core::AppConfig cfg;
        cfg.windowDesc = gs::window::WindowDesc{"GSAW", 1280, 720, false, true, 32};
#ifdef _WIN32
        ...
        gs::core::Application app(cfg);

        auto uiLayer = std::make_unique<gs::ui::UILayer>();
