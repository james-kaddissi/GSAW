...
public:
  explicit GSAWAppLayer(gs::ui::UILayer &uiLayer, AudioLayer &audioLayer)
  ...
  void onAttach(gs::core::AppContext &ctx) override {
    ...
    setupChannelRack();
    m_channelRackView = ChannelRackView::create(
        m_audio.getEngine(), m_audio.getEngine()->getChannelRack());

    auto probeFn = [this](const std::wstring &path, std::string &outName,
                          bool &outIsInstrument) -> bool {
    ...
    m_browserPanel =
        BrowserPanel::create(m_owner, m_audio.getEngine(), std::move(probeFn));

    m_uiLayer.getLayoutRoot().setOnDrop(
        [this](DragPayload p) { m_browserPanel->handleDrop(std::move(p)); });
    fpsText = ui::Text("FPS: --").size(14).color({0.7f, 0.7f, 0.7f, 1});
    statusText =
        ui::Text("Active")
            .size(14)
            .color({0.5f, 0.9f, 0.5f, 1.0f})
            .style(StyleStates::Muted, gs::ui::style::StyleOverrides::TextColor(
                                           {0.9f, 0.4f, 0.4f, 1.0f}));

    auto audioProvider = [this](UIWaveformSource src, int count,
    ...
    auto btnMin = ui::Button("_")
                      .width(46)
                      .height(32)
                      .colors({0.15f, 0.15f, 0.15f, 1},
                              {0.25f, 0.25f, 0.25f, 1}, {0.1f, 0.1f, 0.1f, 1})
    ...
    auto btnClose = ui::Button("X")
                        .width(46)
                        .height(32)
                        .colors({0.15f, 0.15f, 0.15f, 1}, {0.8f, 0.2f, 0.2f, 1},
                                {0.6f, 0.1f, 0.1f, 1})
  ...
  void onEvent(gs::core::AppContext &ctx,
               const gs::core::AppEvent &e) override {
...
private:
  void setupChannelRack() {
    ...
    TrackId synthTrackId = engine->createTrack(
        "Synth", 2, {gs::audio::InputSource::Kind::None, 0, 0}, kMasterBusId);
...
int main() {
  try {
    ...
    cfg.windowDesc =
        gs::window::WindowDesc{"GSAW", 1280, 720, false, true, 32};
#ifdef _WIN32
